...
class BaseTranslator(ABC):
    ...
    def _safe_translate_chunk(
        ...
        try:
            ...
            if cached_result is not None:
                ...
                logger.warning(f"Ignoring invalid translation cache: {cache_key}")
                self._cache.delete(cache_key)
            ...
            if self._is_cacheable_result(result):
                ...
                logger.warning(
                    f"Skipping translation cache write for invalid result: {cache_key}"
                )
            return result
    ...
    def _is_cacheable_result(self, result: List[SubtitleProcessData]) -> bool:
        """Return False for empty, ERROR-like, or partially failed translations."""
        try:
            for data in result:
