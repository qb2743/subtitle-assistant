...
class TTSConfig:
    ...
    max_retries: int = 3  # 速率限制/网络错误重试次数（429 等由 SDK 自动重试）
    use_cache: bool = True  # 是否Using cache
    # 本地 Gradio 服务（Dots-TTS / VoxCPM）相关
    start_script: str = ""  # 本地服务启动脚本路径（可选；留空则要求服务已手动启动）
    service_start_timeout: int = 180  # 本地服务启动超时（秒）
    voxcpm_version: str = "v2"  # VoxCPM Gradio 参数形态：v2 / v1 / hf
