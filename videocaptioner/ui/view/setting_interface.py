    ...
    def __createLLMServiceCards(self):
        ...
        for service, config in service_configs.items():
            ...
            if service == LLMServiceEnum.OLLAMA:
                api_key_placeholder = ""
            elif service == LLMServiceEnum.ANTHROPIC:
            ...
            api_base_card = LineEditSettingCard(
                ...
                self.tr("Base URL"),
                self.tr(
                    f"输入 {service.value} Base URL。"
                    f"OpenAI 兼容接口若只填主机地址可省略 /v1，运行时会自动补全；"
                    f"Anthropic/智谱等请按提供商文档填写完整路径。"
                ),
                config["default_base"],
                self.llmGroup,
            )
            ...
            # 设置只读状态：仅部分提供商允许自定义 Base URL（含 Anthropic 中转/兼容网关）
            if service not in [
                LLMServiceEnum.OPENAI,
                LLMServiceEnum.OLLAMA,
                LLMServiceEnum.LM_STUDIO,
